...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.msXbox.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ms-xboxes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="game">{{ trans('cruds.msXbox.fields.game') }}</label>
                <input class="form-control {{ $errors->has('game') ? 'is-invalid' : '' }}" type="text" name="game" id="game" value="{{ old('game', '') }}" required>
                @if($errors->has('game'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('on_sale') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="on_sale" id="on_sale" value="1" {{ old('on_sale', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price_on_sale') ? 'is-invalid' : '' }}" type="number" name="price_on_sale" id="price_on_sale" value="{{ old('price_on_sale', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('release_date') ? 'is-invalid' : '' }}" type="text" name="release_date" id="release_date" value="{{ old('release_date', '') }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('pre_order') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="pre_order" id="pre_order" value="1" {{ old('pre_order', 0) == 1 ? 'checked' : '' }}>
